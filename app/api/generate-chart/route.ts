  ...
  try {
    ...
    if (!apiKey) {
      ...
      return NextResponse.json(
        {
          error:
            "OpenAI API 키가 설정되지 않았습니다. 설정 메뉴에서 API 키를 입력해주세요.",
        },
        { status: 400 }
      );
    }
    ...
    const shouldExtractImageData =
      image &&
      extractImageDataKeywords.some((keyword) =>
...
${
  shouldExtractImageData ? "5" : "6"
}. Choose the most appropriate chart type based on the DATA and request. ECharts supports many chart types:
   ...
   - radar: for multi-dimensional data comparison (spider chart)
   - sankey: for flow diagrams showing relationships between nodes
   - funnel: for conversion funnels or process flows
   - gauge: for single value indicators (speedometer style)
   - treemap: for hierarchical data visualization
   - sunburst: for hierarchical data with concentric circles
   - heatmap: for matrix data visualization
   - candlestick: for financial data (OHLC - Open, High, Low, Close)
   - boxplot: for statistical data distribution
   - graph: for network diagrams and relationship graphs
   - parallel: for multi-dimensional data with parallel coordinates
   - tree: for tree structure visualization
   - themeRiver: for event flow over time
   - map: for geographic data visualization
   - 3D charts: bar3D, line3D, scatter3D, surface, etc.
   You can use ANY ECharts chart type. Choose the most appropriate one based on the user's request and data structure.
7. Include proper labels, tooltips, and styling
...
${
  previousChartConfig
    ? `\nThe user wants to improve/modify an existing chart. Previous chart config: ${JSON.stringify(
        previousChartConfig
      )}`
    : ""
}
...
Return ONLY the JSON object, nothing else.`;
    ...
    if (image) {
      ...
      if (shouldExtractImageData) {
        // 이미지 데이터 추출 모드
        console.log(
          "📸 이미지 데이터 추출 모드 - 이미지에서 데이터를 추출하여 차트 생성"
        );
        messages.push({
          role: "user",
          content: [
        ...
        // 스타일 참고 모드 (기본)
        console.log(
          "📸 이미지 참고 모드 - 차트 스타일/유형 참고용 (데이터는 텍스트에서 추출)"
        );
        messages.push({
          role: "user",
          content: [
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        ...
        if (
          ...
          throw new Error(
            "OpenAI API 사용량 한도를 초과했습니다. 결제 정보를 확인하거나 사용량을 확인해주세요. (https://platform.openai.com/account/billing)"
          );
        }
      }

      throw new Error(
        `OpenAI API 오류: ${response.status} ${errorData.error?.message || ""}`
      );
    }
